...
for i in 1..20 do
  create_group = Group.create!([title: "Group no.#{i}", description: "這是用種子建的第 #{i} 個討論版", user_id: "1"])
  create_account.join!(create_group)
  for k in 1..30 do
    Post.create!([group_id: "#{i}", content: "這是用種子建的第 #{k} 篇留言", user_id: "1"])
  end
end
